...
import MinhT.dto.OrderDto;
import MinhT.dto.Phone;
import MinhT.dto.UserDTO;
import MinhT.dto.orderDetailDTO;
...
//            System.out.println(item+"items");
    ...
    public List<OrderDto> getAllOrder() {
        ...
        ResultSet rs = null;
        UserDTO userDTO;
        String query = "  select [oderID]\n"
                + "      ,[date]\n"
                + "      ,[totalPrice]\n"
                + "      ,[name]\n"
                + "      ,[userID]\n"
                + "      ,[status] from TblOders;";
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                listOrder.add(new OrderDto(rs.getString(1), rs.getDate(2), rs.getFloat(4), rs.getString(5), rs.getString(6), rs.getInt(7)));
            }
        } catch (Exception e) {
    ...
    public List<orderDetailDTO> getAllOrderDetail() {
        ...
        ResultSet rs = null;
        UserDTO userDTO;
        String query = "   SELECT[oderDetailID]\n"
                + "      ,[name]\n"
                + "      ,[totalPrice]\n"
                + "      ,[quantity]\n"
                + "      ,[productID]\n"
                + "      ,[oderID]\n"
                + "  FROM [Assisgment].[dbo].[TblOderDetail]";
        try {
            conn = DBUtils.getConnection();
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
            while (rs.next()) {
                listOrder.add(new orderDetailDTO(rs.getString(1), 
                       rs.getString(2), 
                        rs.getFloat(3),
                        rs.getString(4),
                        rs.getString(5)));
